              ...
              {item.keyPoints && (
                <ul className="mt-2.5 ml-12 space-y-1.5">
                  {item.keyPoints.map((k) => (
                    <li key={k} className="flex items-center gap-2 text-xs text-white/55">
  ...
  const panelContent = (
    <>
      ...
      <div className="flex-1 overflow-y-auto overscroll-contain p-5 space-y-5">
        {/* Subtitle */}
        <p className="text-sm text-white/45 leading-relaxed">{item.subtitle}</p>
        ...
        <div>
          <div className="text-[9px] font-bold uppercase tracking-widest text-white/22 mb-2 font-mono">
        ...
        {item.keyPoints && item.keyPoints.length > 0 && (
          <div>
            <div className="text-[9px] font-bold uppercase tracking-widest text-white/22 mb-2.5 font-mono">
              Key Points
            </div>
        ...
        <div className="pt-3 border-t border-white/[0.06]">
          <div className="text-[9px] font-mono text-white/18 mb-1 uppercase tracking-widest">Permalink</div>
          <button
            onClick={copyUrl}
            className="text-[10px] font-mono text-white/28 hover:text-white/55 transition-colors text-left"
          >
            ?view={item.id}
          </button>
        </div>
    ...
    <section id="search" className="section" aria-labelledby="search-heading">
      ...
      <FadeIn delay={0.07}>
        <div className="relative max-w-2xl mx-auto mb-4">
          <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-4.5 h-4.5 text-white/25 pointer-events-none" />
          <input
            ref={inputRef}
            id="kb-search"
            type="search"
            ...
            className="w-full pl-11 pr-10 py-3.5 sm:py-4 rounded-2xl glass border border-white/[0.09] text-white placeholder:text-white/22 text-sm sm:text-base focus:outline-none focus:border-neon-cyan/40 focus:shadow-[0_0_0_3px_rgba(77,255,255,0.07)] transition-all"
            autoComplete="off"
        ...
        <div className="pl-5 border-l border-white/[0.06] relative">
          {results.length === 0 ? (
            ...
            >
              ...
              <p className="text-sm mt-1 text-white/15">Try a different keyword or tag</p>
            </motion.div>
          ) : (
            <>
        ...
        <AnimatePresence mode="wait">
          {active ? (
            ...
            >
              ...
              <p className="text-sm text-white/28 font-medium">
                Select any entry to open the wiki view
              </p>
              <p className="text-xs text-white/15 mt-2">
                Deep-linkable · Shareable · Copyable
              </p>
              <div className="mt-6 flex items-center gap-2 text-[10px] font-mono text-white/15">
