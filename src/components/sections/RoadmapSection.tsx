      ...
      {bars.map((b) => (
        <div
          ...
          className={`h-3 w-1.5 rounded-full transition-all ${
            b <= score ? "bg-neon-cyan" : "bg-white/10"
          }`}
    ...
    <>
            ...
            >
              <thead>
                <tr className="border-b border-white/5">
                  <th className="text-left px-5 py-4 text-xs font-bold text-white/30 uppercase tracking-wider w-36">
                    Feature
                  </th>
                  {tools.map((t) => (
                    <th
                      key={t}
                      className="px-3 py-4 text-center text-xs font-bold text-white/60 uppercase tracking-wide"
                    >
                      {t}
                    </th>
                  ))}
      ...
      >
        ...
        <div className="relative">
          ...
          <div className="absolute left-6 md:left-1/2 top-0 bottom-0 w-px bg-gradient-to-b from-neon-cyan via-neon-purple to-neon-green opacity-20 hidden sm:block" />
                      ...
                      <div className="p-6">
                        ...
                        >
                          {item.items.map((it) => (
                            <li
                              key={it}
                              className={`flex items-center gap-2 text-sm text-white/60 ${i % 2 === 1 ? "md:flex-row-reverse" : ""}`}
                            >
                              <div
